...
class GoogleSerperAPIWrapper():
    ...
    def _parse_results(self, results):
        ...
        # Patched by Georgi
        if int(self.k / 2) % 2 != 0:
            print(f'[SERPER] Length of snippets: {len(snippets)} => Appending empty snippet to make it even')
            element = {"content":"No good Google Search Result was found","source":"None"}
            snippets.append(element) # add error object to make it even
            self.k += 1
        # end of patch
    ...
    async def run(self, queries):
        ...
        # Patched by Georgi
        if len(snippets_list) % 2 != 0:
            print(f'[SERPER] Length of snippets_list: {len(snippets_list)}')
            snippets_list.append([{ "content": "No good Google Search Result was found", "source": "None" }]) # add empty evidence object to make it even
            print(f'[SERPER] CORRECTED len of snippets_list: {len(snippets_list)}')
        # end of patch
