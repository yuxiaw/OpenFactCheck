...
class AtomicFactGenerator(object):
    def __init__(self, key_path, demon_dir, gpt3_cache_file=None):
        ...
        # changed gpt3_cache_file to chatgpt in call
        self.openai_lm = OpenAIModel("ChatGPT", key_path=key_path, cache_file = gpt3_cache_file) 
        # self.openai_lm = LLaMA3(model_name="meta-llama/Meta-Llama-3-8B-Instruct", cache_file=gpt3_cache_file.replace("ChatGPT.pkl", "inst-llama3-8B.pkl"))
    ...
    def get_atomic_facts_from_paragraph(self, paragraphs, cost_estimate=None):
        ...
        # here they constrain the input sentences are checkworthy, remove potentially meaningless sents
        # if self.is_bio == False, then not self.is_bio is True, no sentences will be as input, it would be empty list inputting into self.get_init_atomic_facts_from_sentence([]).
        ...
        # if just estimate cost, they will not prompt llms such as ChatGPT to split sentence into Atomic claims
        if cost_estimate:
...
def detect_initials(text):
    """This pattern is looking for two uppercase letters separated by an optional space and followed by a period.This pattern is used to match initials like "J. R." or "A.B." in a text."""
    pattern = r"[A-Z]\. ?[A-Z]\."
