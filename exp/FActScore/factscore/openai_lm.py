...
# rewrite this function to match the openai update
def call_ChatGPT(message, model_name="gpt-3.5-turbo", max_len=1024, temp=0.7, num_retries=3, waiting_time = 1):
    # call GPT-3 API until result is provided and then return it
    logging.info("call model ChatGPT...")
    response = None
    num_rate_errors = 0
    for _ in range(num_retries):
        try:
            r = openai.chat.completions.create(model=model_name, messages=message)
            response = ''
    ...
    if response is None:
        logging.info("Failing to call ChatGPT.")
    return response
